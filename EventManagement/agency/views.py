...
def preference_view(request):
    ...
    agency_info = Agency.objects.filter(agent_username=request.user.username)
    ...
    print(agency_company.agency_company_name)
    works = Event.objects.filter(event_username=agency_company.agency_company_name)
    ...
    return render(request, 'agency_preference.html', {'agency_info': agency_info, 'form': form, 'reviews': reviews, 'requests': requests, 'payments': payments,
                                                      'works': works}, )
...
def agency_view(request):
    ...
    if request.method == 'POST' :
        ...
        if form.is_valid():
            ...
            obj.save()
            if obj.agency_employee_number == None:
                obj.agency_studio_size = "i do not care"
            elif obj.agency_employee_number <= 10:
